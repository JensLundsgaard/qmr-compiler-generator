...
fn emit_impl_arch_methods(arch: &Option<ArchitectureBlock>) -> TokenStream {
    ...
    let custom_from_file_body = match arch {
        ...
        Some(arch) => {
            let set_extra_fields = arch.data.fields.iter().map(|(name, ty)| {
                let field_name = syn::Ident::new(name, Span::call_site());
                let ty_quote = emit_type(ty);
                quote! {
                    #field_name : from_value(parsed[#name].clone()).expect(&format!("Error parsing field: {}", #name.to_string())),
                }
            });
            quote! {
...
fn emit_solve_function(imp: &ImplBlock) -> TokenStream {
    ...
    quote! {
        fn my_solve(c : &Circuit, a : &CustomArch) -> CompilerResult<#imp_struct_name> {
            return backend::solve(c, a, &|s| available_transitions(a, s), realize_gate, custom_step_cost, Some(mapping_heuristic));
    }
    }
}
...
fn emit_sabre_solve_function(imp: &ImplBlock) -> TokenStream {
    ...
    quote! {
        fn my_sabre_solve(c : &Circuit, a : &CustomArch) -> CompilerResult<#imp_struct_name> {
            return backend::sabre_solve(c, a, &|s| available_transitions(a, s), realize_gate, custom_step_cost, Some(mapping_heuristic));
    }
    }
}
